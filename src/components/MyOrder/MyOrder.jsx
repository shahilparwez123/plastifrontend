...
import { FiArrowLeft, FiBox, FiMapPin, FiUser } from 'react-icons/fi'
...
const MyOrder = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await axios.get('https://plastibackend.onrender.com/api/orders', {
...
const getPaymentMethodDetails = (method) => {
    if (method === 'cod') {
        return {
            label: 'COD',
            class: 'bg-yellow-900/20 text-yellow-400 border-yellow-500/30'
        };
    }
    ...
    if (method === 'online') {
        return {
            label: 'Online',
            class: 'bg-green-900/20 text-green-400 border-green-500/30'
        };
    }

    return {
        label: method || 'Unknown',
        class: 'bg-gray-900/20 text-gray-400 border-gray-500/30'
    };
};
    ...
    return (
        <div className=' min-h-screen bg-gradient-to-br from-[#1a120b] via-[#2a1e14] to-[#3e2b1d] py-12 px-4 sm:px-6 lg:px-8'>
            <div className=' mx-auto max-w-7xl'>
                ...
                <div className=' bg-[#4a3b3b]/80 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border-2 border-amber-500/20 '>
    ...
    <th className="p-4 text-left text-amber-400">Status</th>
        ...
        return (
            ...
            transition-colors group'>
                <td className=' p-4 text-amber-100 font-momo text-sm'>
                    #{order._id?.slice(-8)}
                    </td> 
